...
def parse_ticket_vdv(ticket_bytes: bytes, context: "vdv.ticket.Context") -> VDVTicket:
    ...
    if getattr(settings, 'DEBUG', False):
        ...
        try:
            # Try to parse just the envelope first
            try:
                motics = vdv.Motics.parse(ticket_bytes)
                envelope = vdv.EnvelopeV2.parse(motics.application_data)
                print(f"DEBUG: Parsed envelope with Motics")
            except vdv.motics.NotAMoticsException:
                motics = None
                envelope = vdv.EnvelopeV2.parse(ticket_bytes)
                print(f"DEBUG: Parsed envelope without Motics")
            
            print(f"DEBUG: Envelope parsed - signature length: {len(envelope.signature)}")
            print(f"DEBUG: Envelope - residual data length: {len(envelope.residual_data)}")
            
            # In development mode, try to parse the ticket data directly from residual_data
            # The residual_data typically contains the actual ticket information
            try:
                print("DEBUG: Trying to parse ticket data from residual_data")
                ticket_data = vdv.VDVTicket.parse(envelope.residual_data, context)
                print(f"DEBUG: Successfully parsed VDV ticket from residual data!")
            ...
            except Exception as ticket_parse_error:
                print(f"DEBUG: Failed to parse from residual_data: {ticket_parse_error}")
                # Try parsing from the full signature + residual_data
                try:
                    combined_data = envelope.signature + envelope.residual_data
                    print(f"DEBUG: Trying to parse from combined signature+residual data (length: {len(combined_data)})")
                    ticket_data = vdv.VDVTicket.parse(combined_data, context)
                    ...
                    print(f"DEBUG: Successfully parsed VDV ticket from combined data!")
                    return VDVTicket(
                        root_ca=mock_cert_data,
                        issuing_ca=mock_cert_data,
                        envelope_certificate=mock_cert_data,
                    ...
                    print(f"DEBUG: Failed to parse from combined data: {combined_error}")
